    ...
    for i = 1:length(subDirs)
        ...
        for f = 1:size(targetFiles, 1)
            ...
            % --- A. LOAD & CLEAN (Stitching Version) ---
                ...
                % detectionState = [0, 0];  % For >3s of walking
                detectionState = 0;
                ...
                for s = 2:totalSamples
                    ...
                    if mod(s, stepSize) == 0 && s >= windowSize
                        ...
                        % Store plot metrics (These will now grow correctly!)
        ...
        if ~isempty(fieldnames(plotData))
            try
                ...
                % Add this as Subplot 1 (shift others down by 1):
                ...
                for si = 1:length(sides)
                    plot(plotData.(sides{si}).T_vec, plotData.(sides{si}).peakF, 'Color', colors{si}, 'LineWidth', 1.2);
                end
                ...
                for si = 1:length(sides)
                    plot(plotData.(sides{si}).T_vec, plotData.(sides{si}).maxPk, 'Color', colors{si});
                end
                ...
                for si = 1:length(sides)
                    plot(plotData.(sides{si}).T_vec, plotData.(sides{si}).ampVal, 'Color', colors{si});
                end
                ...
                h_leg = [];
                for si = 1:length(sides)
                    a_h = area(plotData.(sides{si}).time, plotData.(sides{si}).y_true, 'FaceColor', colors{si}, 'FaceAlpha', 0.1, 'EdgeColor', 'none');
                    p_h = stairs(plotData.(sides{si}).time, plotData.(sides{si}).y_pred, 'Color', colors{si}, 'LineWidth', 1.5);
                    h_leg = [h_leg, a_h, p_h];
                end
        ...
        % else
            % fprintf('  --> No plotData for %s, skipping plot.\n', folderName);
        end
    end
end
