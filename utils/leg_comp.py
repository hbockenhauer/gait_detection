...
Can only to run on the clinical and multiple activities
datasets. 
"""
...
SAVE_RESULTS = True 
PRINT_STATS = True 
PLOT_SAVE_FOLDER = "Leg_algorithm" 
PLOT_PRED = True
#############################

PLOT_SAVE_PATH = os.path.join(PLOTS_DIR, PLOT_SAVE_FOLDER)
...
def process_leg(rl_merged: pd.DataFrame, save_results: bool = False):
    ...
    for subject, grp in rl_merged.groupby('subject', sort=True):
        ...
        if PLOT_PRED == True: 
            ...
            out_path = os.path.join(PLOT_SAVE_PATH, f"{subject}_leg_pred.png")
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 80}")
    print(f"  Running the Leg Algorithm Check on pooled data ({len(DATA_PATHS)} dataset(s))")
    print(f"{'=' * 80}")
