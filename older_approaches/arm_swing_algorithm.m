...
for i_imu = 1:nr_imu
    ...
    [coeff_av, ang_vel_pca] = pca(ang_vel_imu(:,1:2)-mean(ang_vel_imu(:,1:2),"omitmissing"));
    if  coeff_av(2,1) <0
    ...
    if window*1.5 < length(angle_pca)
        ...
        for i_w = 1:(steps):(length(angle_pca) - window)
            y = fft((angle_pca(i_w:i_w + window,1)-mean(angle_pca(i_w:i_w + window,1),"omitmissing")),nfft);
        ...
        for i_wp = 1:(steps_pk):(length(angle_pca) - window_pk)
            if i_wp ==1
                cycle_time = 1/median(swing_freq(1:2),"omitmissing");
                if sum(idx_remove(1:2))>0
                    cycle_time = nan;
                end
            elseif i_wp == floor(length(angle_pca)/steps_pk)*steps_pk - window_pk +1 %last window
                cycle_time = 1/ mean(swing_freq((round(i_wp/steps_pk) +1) *2 -2:end),"omitmissing");
                if sum(idx_remove((round(i_wp/steps_pk) +1) *2 -2:length(idx_remove)))>1
                    cycle_time = nan;
                end
            else
                cycle_time = 1/ mean(swing_freq((round(i_wp/steps_pk) +1) *2 -2:(round(i_wp/steps_pk) +1) *2 +1),"omitmissing");
        ...
        if ~all(isnan(cycle_time_k))
            ...
            if ~isempty(idx_all_pks) && length(idx_all_pks) >1
                ...
                for i_d = 1:length(idx_all_pks)-1
                    dlt(i_d,1) = (idx_all_pks(i_d+1) - idx_all_pks(i_d))> 2*mean(cycle_time_k,"omitmissing")*fs;
                end
                ...
                if ~isnan(start_idx)
                    ...
                    regularity_angle = mean(acf_max,"omitmissing");
                    [acf_max_av] = auto_cor_wrist(ang_vel_pca(start_idx(1):end_idx(end),1),fs);
                    regularity_ang_vel = mean(acf_max_av,"omitmissing");
                    ...
                    % swings
                    avg_pk_vel_forward = mean(ang_vel_forward,"omitmissing");
                    avg_pk_vel_backward = mean(ang_vel_backward,"omitmissing");
...
if nr_imu == 2
    ...
    if length(arm_swing_r.amplitude)<3 || length(arm_swing_l.amplitude)<3
        ...
        if sum(both_swing_l)<3 || sum(both_swing_r)<3 || perc_both_swing <60
            ...
            % asymmetry index
            avg_ampl_l = mean(ampl_l_asym,"omitmissing");
            avg_ampl_r = mean(ampl_r_asym,"omitmissing");
            avg_peak_vel_l = mean(peak_vel_l_asym,"omitmissing");
            avg_peak_vel_r = mean(peak_vel_r_asym,"omitmissing");
            ...
            cross_cor_max = mean(cross_cor_max_cc,"omitmissing");
        end
